...
int main() {
    ...
    // Create a bigger quad positioned where we know it was visible
    float vertices[] = {
        // Position              
        -2.0f, -2.0f,  0.7f,    // bottom left  (using the z value where we saw it ~0.7)
         2.0f, -2.0f,  0.7f,    // bottom right (made quad bigger: -2 to 2 instead of -1 to 1)
         2.0f,  2.0f,  0.7f,    // top right
        -2.0f,  2.0f,  0.7f     // top left
    };
    ...
    // Start at the position where we know the quad was visible
    float cameraDistance = 0.5f;
    float cameraTheta = 0.0f;
    float cameraPhi = 0.3f;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (frameCount++ % 60 == 0) {
            std::cout << "Camera pos: (" << camX << ", " << camY << ", " << camZ << ")"
        ...
        // Improved view matrix calculation
        ...
        float lenF = std::sqrt(forward[0]*forward[0] + forward[1]*forward[1] + forward[2]*forward[2]);
        if (lenF > 0) {
            forward[0] /= lenF;
            forward[1] /= lenF;
            forward[2] /= lenF;
